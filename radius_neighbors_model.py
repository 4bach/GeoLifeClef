...
class RadiusNeighborsModel(Classifier):
    ...
    def __init__(self, radius=1.0, weights='uniform', p=2, metric='minkowski', how_outliers='most_common', ranking_size=30):
        """
           :param radius: the number of neighbors for predicting class probabilities
           ...
           :param metric: the distance metric to use for the tree. The default
               metric is minkowski, and with p=2 is equivalent to the standard
               Euclidean metric. Choices are:
           ...
           :param how_outliers: how are outlier examples treated: the examples
              which do not have any neighbors within the given radius. Possible
              values are:
            - 'most_common': returns the most common label in the training set
            - 'random': returns a random label from the training set
        """
        self.radius = radius
